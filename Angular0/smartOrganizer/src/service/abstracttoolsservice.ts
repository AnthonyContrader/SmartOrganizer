 ...
 */
export abstract class AbstractServiceTool<DTO> implements Service<DTO> {
    ...
    auth() {
      ...
      usura(dto: DTO): Observable<DTO> {
        return this.http.put<DTO>('http://localhost:' + this.port + '/' + this.name + '/' + 'api' + '/' + this.type +"/usura", dto , {
          headers: {
            Authorization : this.auth()
          }
        });
